...
// 20260703             SyclQueueEventWatcher::getInstance().SetEvent(e);
//  各処理でauto e = を行わないとダメ
...
class SyclQueueEventWatcher {
private:
    std::vector<sycl::event> events;
    //sycl::event events[SYCL_EVENTS_SIZE] = {};
    //std::array<sycl::event, SYCL_EVENTS_SIZE> events = {};
    SyclQueueEventWatcher();
    SyclQueueEventWatcher(const SyclQueueEventWatcher&) = delete;
    SyclQueueEventWatcher& operator=(const SyclQueueEventWatcher&) = delete;
    ~SyclQueueEventWatcher() {}
    int get_events_size();
public:
    static SyclQueueEventWatcher& getInstance();
    void SetEvent(sycl::event);
    void WaitForSubmit();
};

//public static SyclQueueEventWatcher Instance => _instance ?? (_instance = new SyclQueueEventWatcher());
...
/*
void wait_for_submit(queue_ptr stream){
    int chk_cnt = 0;
    size_t s = stream->get_wait_list().size();
    while(s < 20){
...
void ggml_sycl_looper(
        ...
        SubmitFunc submit_func)
{
    sycl::device dev = cgh.get_device();
    const int max_work_group_size = dev.get_info<sycl::info::device::max_work_group_size>();
    ...
    while(i < group_cnt){
        ...
        //if(wait_cnt++%SYCL_QUEUE_WAIT_TIMMING==0){
        //    //GGML_SYCL_DEBUG("[SYCL] %s wait()\n", __func__);
        //    //SYCL_CHECK(CHECK_TRY_ERROR((cgh.wait()));
        //    cgh.wait();
        //}
